...
/// </para>
/// </remarks>
public class UIInputHandler : MonoBehaviour, IUserInputHandler
{
    ...
    void Start()
    {
        if (_graphicRaycaster == null)
        {
            Logger.LogMessage(LogSeverityLevel.Error, $"Missing GraphicRaycaster reference on {nameof(UIInputHandler)}.");
            enabled = false;
            return;
        }
        ...
        if (_eventSystem == null)
        {
            Logger.LogMessage(LogSeverityLevel.Error, $"Missing EventSystem reference on {nameof(UIInputHandler)}.");
            enabled = false;
            return;
        }
    }
    ...
    /// </summary>
    /// <param name="touchPosition">The screen position at which to check for an UI element</param>
    /// <returns>
